...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--data-path", type=Path, required=True, help="CSV file with OHLCV data")
...
def main() -> None:
    ...
    frame = pd.read_csv(config.dataset.data_path)
    frame = ensure_datetime_index(frame)
    frame = frame.reset_index()
    train_frame, val_frame = split_train_validation(frame, config.dataset.train_ratio)
    ...
    train_dataset = CandlestickDataset(train_frame, window, normalize=config.dataset.normalize)
    stats = None
